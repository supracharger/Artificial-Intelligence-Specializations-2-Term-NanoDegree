...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Add Action Level
        actionsLvl = set()
        self.a_levels.append(actionsLvl);
        # Loop each literal State from Parent
        for literal in self.s_levels[level]:
            # Loop all Actions
            for act in self.all_actions:
                # Get the CondType Group according to the sign of the literal.symbol
                condType = act.precond_pos if literal.is_pos else act.precond_neg
                # if literal Cond. is in action PreCondType
                if literal.symbol in condType:
                    act = PgNode_a(act)             # Create PgNodeAction obj.
                    act.parents.add(literal);       # Link Literal State Parent
                    actionsLvl.add(act)             # Add to a_levels Set
                    literal.children.add(act)       # Add to Prev. Nodes ChildrenSet
    ...
    def add_literal_level(self, level):
        ...
        # Add Literal State Level
        literalLvls = set();
        self.s_levels.append(literalLvls)
        # Loop by Parent Actions
        for act in self.a_levels[level - 1]:
            # Add Pos. Effects
            for effect in act.action.effect_add: 
                act.children.add(PgNode_s(effect, True))
            # Add Neg. Effects
            for effect in act.action.effect_rem: 
                act.children.add(PgNode_s(effect, False))
            # Loop by Children that where just Added
            for s in act.children:
                literalLvls.add(s);     # Add LiteralState to levels Set
                s.parents.add(act);     # Link LiteralState Parent Action
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        CITED oliver.graeser: https://discussions.udacity.com/t/how-to-pass-the-test-inconsistent-effects-mutex/300233/2?u=suprachargers2d2n
        Inconsistent effects are 'opposite effects' - i.e., same symbol but opposite sign. 
        Your code must be marking some additional ones at mutually exclusive. If that does 
        not help, please post your code (inside triple backtics and spoiler tags)
        ...
        actions = [node_a1.action, node_a2.action]                          # List 2 Actions
        effectGroups = [[a.effect_add, a.effect_rem] for a in actions]      # 2Effects of the 2Actions Grouped
        # Loop EffectGroups action1Effects[], then action2Effects[]
        for i, effectG in enumerate(effectGroups):
            other = 1 if i==0 else 0        # BiPole: Will Always be the Opposite 
            # Loop Effects [remOther[i] in add], the [addOther[i] in rem]
            for pole in range(2):
                biPole = 1 if pole==0 else 0                # BiPole: Will Always be the Opposite 
                for e in effectGroups[other][biPole]:       # Foreach current effect[] in action1Effects
                    if e in effectG[pole]:                  # If Not Found 'e' in action2Effects
                        return True                         # RETURN: True (Inconsistent)
        # RETURN: False (Consistant)
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        actions = [node_a1.action, node_a2.action]                          # List 2 Actions
        effectGroups = [[a.effect_add, a.effect_rem] for a in actions]      # 2Effects of the 2Actions Grouped
        condGroups = [[a.precond_pos, a.precond_neg] for a in actions]      # 2PreCond of the 2Actions Grouped
        # Loop EffectGroups: action1EffectGroup, the action2EffectGroup
        for i, effectGrp in enumerate(effectGroups):
            other = 1 if i==0 else 0                    # BiPole: Will Always be the Opposite 
            condOther = condGroups[other]               # Other actions Preconditions
            # Loop each effect[] list in Group. Pos. then Neg.
            for pole, effect in enumerate(effectGrp):
                biPole = 1 if pole==0 else 0                    # BiPole: Will Always be the Opposite 
                # Loop each one Effect
                for e in effect:
                    # if oneEffect found in the biPole of the other ActionsPreCond: True
                    if e in condOther[biPole]:
                        return True;                            # RETURN True (Interference)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Cited "emindeniz": https://discussions.udacity.com/t/problem-with-test-competing-needs-mutex/227344/2?u=suprachargers2d2n
        #       & Help from Mentor Miguel Martinez

        # Loop Parents in NodeA1, really only one
        for parent1 in node_a1.parents:
            # Loop Parents in NodeA2, really only one
            for parent2 in node_a2.parents:
                # Test the pair of actions for mutual exclusion
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # For Same Literal Symbol: "returning True if: one node is the negation of the other, and False otherwise"
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        result = False
        # Loop Parents in NodeS1, really only one
        for parent1 in node_s1.parents:
            # Loop Parents in NodeS1, really only one
            for parent2 in node_s2.parents:
                if parent1.is_mutex(parent2):
                    result = True
        # "If one parent action can achieve both states, should NOT be inconsistent-support mutex, 
        #       even if parent actions are themselves mutex"
        if (result):
            states2 = [node_s1, node_s2]        # Group States to iterate.
            # Loop 1 of each 2 States
            for state in states2:
                # Loop Parents of each State
                for parent1 in state.parents:
                    ctr = 0
                    # Check if the ParentEffects are both included in the 2 literals
                    # Loop 1 of each 2 States 
                    for s in states2:
                        # Get the right sign Effect
                        effects = parent1.action.effect_add if (s.is_pos) else parent1.action.effect_rem
                        # if literal is in Parent Effect
                        if s.symbol in effects:
                            ctr += 1
                    # False (NOT inconsistent-support): If Both Literals in ParentsEffect
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0
        # Loop each Goal & Estimate the levelCost
        for goal in self.problem.goal:
            found = False
            # Loop by Literal Level
            for level, sLvl in enumerate(self.s_levels):
                if found: break                         # If found Estimate Break
                # Loop each node in Level
                for node in sLvl:
                    # LevelCostEst: Find the 1st Pos. occur. of the literalGoal & Return
                    #               What Level it was found
                    # If found 1st Pos. Occur.
                    if goal==node.symbol and node.is_pos:
                        level_sum += level                      # Add level to sum
                        found = True                            # Set Bool to Break out of 2nd Upper loop
                        break                                   # Break
        # RETURN        
        return level_sum
